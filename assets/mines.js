...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a31d33aa";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a31d33aa">'+'<span class="tile-face-a31d33aa tile-front-a31d33aa"></span>'+'<span class="tile-face-a31d33aa tile-back-a31d33aa"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a31d33aa")){return;}
var back=tile.querySelector(".tile-back-a31d33aa");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a31d33aa");if(isMine){tile.classList.add("boom-a31d33aa");}
if(dim){tile.classList.add("dimmed-a31d33aa");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a31d33aa")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a31d33aa");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a31d33aa")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a31d33aa");void panelEl.offsetWidth;panelEl.classList.add("jolt-a31d33aa");}
endRound();}
